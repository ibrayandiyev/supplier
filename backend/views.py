...
from .models import User, Category, CsvArchivo
from .models import Contact, ContactFavorite
from .models import Client, ClientFavorite, ClientAddress
from .models import Stock, StockFavorite
...
from .models import WallType, Castor, Color, DrawerColor, Location, Strip, Lock, Task

from outcome.models import Outcome
from purchase.models import Purchase, Transport
from job.models import Job, JobCandidate

from .forms import UserForm, StockForm, ContactForm, ClientForm, CommandForm

from .resources import ContactResource, LeadResource, ClientResource, StockResource, GroupResource
...
class SupplierDetail(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['child_contacts'] = Contact.objects.filter(parent_id=self.kwargs.get('pk'))
...
class SupplierAdd(CreateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["countries"] = Country.objects.all()
        context["companies"] = Contact.objects.filter(parent=None)
        return context
    def get_success_url(self):
...
class LeadDetail(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['contact'] = contact
        context['child_contacts'] = Client.objects.filter(parent_id=self.kwargs.get('pk'))
...
class LeadContactAdd(CreateView):
    ...
    def get_success_url(self):
        ...
        contact.b_client = contact.parent.b_client
        contact.save()
        return reverse('detail-lead', kwargs={'pk': self.kwargs.get('pk')})
@method_decorator(login_required, name='dispatch')
